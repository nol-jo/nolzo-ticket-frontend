...
import {
  Calendar, MapPin, Clock, Users, ChevronLeft, ChevronRight, Star
} from "lucide-react"
...
const eventData = {
  ...
  schedule: {
    "2025-06-15": ["14:00", "19:00"],
    "2025-06-16": ["14:00", "19:00"],
    "2025-06-17": ["19:00"],
    "2025-06-18": ["14:00", "19:00"],
    "2025-07-18": ["14:00", "19:00"],
    "2025-07-19": ["14:00", "19:00"],
  }
}
...
function BookingCalendar({ selected, onSelect }: { selected: string | null; onSelect: (d: string) => void }) {
  ...
  const isAvailable = (day: number) =>
    Object.keys(eventData.schedule).includes(formatDate(day))
        ...
        {dates.map((d, idx) => {
          ...
          return (
            <button
              ...
              className={`aspect-square rounded text-center ${available ? "text-black" : "text-gray-300"} ${hoverClass} ${isActive ? "bg-blue-600 text-white font-bold" : ""}`}
            >{day || ""}</button>
          )
        })}
      </div>
...
export default function ProductPage() {
  ...
  const times = selectedDate ? (eventData.schedule as Record<string, string[]>)[selectedDate] || [] : [];

  const router = useRouter()
  const handleBook = () => {
    if (!selectedDate || !selectedTime) {
      alert('날짜와 회차를 모두 선택해주세요.')
      return
    }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1">
            <Image src={eventData.poster} alt={eventData.title} width={400} height={533}
